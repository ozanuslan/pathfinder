...
async function draw() {
    ...
    if (!unvisited.isEmpty()) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            else dist = 1.5; // If the vertex is diagonal
    ...
    for (var i = 0; i < gridH; i++) {
        for (var j = 0; j < gridW; j++) {
            ...
            else if (grid[i][j].state == false) clr = [0, 0, 255];
            else clr = [255];
            fill(clr);
            rect(1 + j * rectW, 1 + i * rectH, rectW, rectH);
        }
    ...
    for (var i = 1; i >= -1; i--) {
        for (var j = 1; j >= -1; j--) {
            if (vertex.y + i >= 0 && vertex.y + i < gridH && vertex.x + j >= 0 && vertex.x + j < gridW) {
                if (grid[vertex.y + i][vertex.x + j].state == true) neighbors.push(grid[vertex.y + i][vertex.x + j]);
            }
        }
    }
    return neighbors;
